...
public class CrashHandler implements Thread.UncaughtExceptionHandler {
     ...
     * @throws
     */
    public void init(Context context,AdvisorConnApp app) {
        // 传入app对象，为完美终止app
        this.app = app;
        mContext = context;
    ...
    public void uncaughtException(Thread thread, Throwable ex) {
        if (!handleException(ex) && mDefaultHandler != null) {
            ...
            // 释放资源不能像常规的那样在activity的onDestroy方法里面执行，因为如果出现全局异常捕获
            // activity的关闭有时候是不会再走相关的生命周期函数的（onDesktroy,onStop,onPause等）。
            // 退出app之前需要释放掉的一些资源
